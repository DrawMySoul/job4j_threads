...
class AccountStorageTest {
    ...
    @Test
    void whenAccountsDoesNotExistAndDoTransferThenMustBeFalse() {
        var storage = new AccountStorage();
        storage.add(new Account(1, 100));
        assertThat(storage.transfer(1, 3, 100)).isFalse();
        assertThat(storage.transfer(3, 1, 100)).isFalse();
        var firstAccount = storage.getById(1)
            .orElseThrow(() -> new IllegalStateException("Not found account by id = 1"));
        assertThat(firstAccount.amount()).isEqualTo(100);
    }
    ...
    @Test
    void whenTransferFromAndToTheSameAccountThenMustBeFalse() {
        var storage = new AccountStorage();
        storage.add(new Account(1, 100));
    ...
    @Test
    void whenFromAccountHasAmountLessThenTransferAmountThenMustBeFalse() {
        var storage = new AccountStorage();
        ...
        var secondAccount = storage.getById(2)
            .orElseThrow(() -> new IllegalStateException("Not found account by id = 2"));
        assertThat(firstAccount.amount()).isEqualTo(10);
